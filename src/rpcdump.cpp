...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <deadcoinprivkey> [label] [rescan=true]\n"
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <deadcoinaddress>\n"
            "Reveals the private key corresponding to <deadcoinaddress>.");
...
Value dumppem(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "dumppem <deadcoinaddress> <filename> <passphrase>\n"
            "Dump the key pair corresponding to <deadcoinaddress> and store it as encrypted PEM file."
            + HelpRequiringPassphrase());
