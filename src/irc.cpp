...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #deadcoinTEST2\r");
            Send(hSocket, "WHO #deadcoinTEST2\r");
        } else {
            // randomly join #deadcoin00-#deadcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #deadcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #deadcoin%02d\r", channel_number).c_str());
        }
